...
## 에이전트 운영 규칙 (Agent Operational Rules)

1. **사용자 승인 필수**:
   - 새로운 태스크를 시작하거나 계획을 변경할 때는 **반드시 사용자의 명시적인 승인**을 받아야 합니다.
   - 선제적으로 행동(Proactive)하되, 실행(Execute) 전에는 항상 제안하고 확인받으세요.
   - "진행해", "시작해", "좋아" 등의 명령이 떨어지기 전까지는 `Task` 도구를 호출하거나 계획 문서를 업데이트하지 마십시오.
...
사용자 요청에 따라 문서를 작성할 때 **반드시 아래 위치에 저장**해야 합니다:

| 요청 유형         | 저장 위치                                | 파일 형식             | 예시                                            |
| ----------------- | ---------------------------------------- | --------------------- | ----------------------------------------------- |
| **태스크 추가**   | `.claude/planning/tasks.md`              | 기존 파일에 섹션 추가 | "뉴스 기능 태스크 추가해줘"                     |
| **기획서 작성**   | `doc/project/kcl/화면기획/`              | 신규 마크다운 파일    | "뉴스 화면 기획서 작성해줘" → `뉴스화면기획.md` |
| **기술 문서**     | `doc/project/kcl/기술자료/`              | 신규 마크다운 파일    | "DB 스키마 문서화해줘" → `DB스키마.md`          |
| **테스트 리포트** | `doc/project/kcl/기술자료/테스트리포트/` | 날짜-제목 형식        | `2026-01-15-news-feature-test.md`               |

> ⚠️ **주의**: 루트에 `TASKS.md` 같은 별도 파일을 만들지 마세요. 기존 `.claude/planning/tasks.md`에 통합하세요.
