...
- [x] 다국어 랭킹 리더보드 (12개 언어) ✅ 홈에 통합
- [ ] 실시간 투표 및 로직 (Redis Batching 고려)
- [ ] 회사별 상세 페이지 및 통계

### 2. 통계/분석 페이지 (Analytics) - `/analytics`

**목적**: 투표 데이터 기반의 시각화된 통계 및 트렌드 분석 제공

**핵심 기능**:

- [ ] **실시간 트렌드 차트**: 시간대별/일별 투표 추이 (Line Chart)
- [ ] **소속사별 점유율**: 전체 투표 중 각 소속사 비율 (Pie/Donut Chart)
- [ ] **급상승 소속사**: 최근 24시간/7일 기준 상승폭 Top 5
- [ ] **아티스트별 기여도**: 소속사 내 아티스트별 투표 비중
- [ ] **글로벌 히트맵**: 국가/언어별 투표 분포 (Phase 2)

**UI 구성**:

- **Header**: 기간 필터 (오늘/7일/30일/전체)
- **Main**: 차트 그리드 (반응형 2~3 컬럼)
- **Footer**: 데이터 갱신 시간 표시

**라우트**: `/[locale]/analytics`
**메뉴명**: "통계" (Nav.analytics)

**기술 고려사항**:

- 차트 라이브러리: Recharts 또는 Chart.js (번들 크기 고려)
- 데이터 캐싱: SWR/React Query로 폴링 또는 Realtime 구독
- SSG/ISR: 초기 로드는 정적 생성, 이후 클라이언트 갱신
...
### 4. UI/UX (Integrated Dashboard)

**핵심 철학**: "**Artist First, Company Second**" - 내 가수를 통해 회사에 투표한다.

#### A. 홈 화면 구조 (통합 랭킹 시스템) - ✅ 구현 완료 (배포됨)
