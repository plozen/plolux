...
│ │   ┌──────────────┐         ┌──────────────┐                │ │
│ │   │  [RBW 로고]  │   GAP   │ [IST 로고]   │                │ │
│ │   │   10위 1부   │ 1,200표 │  11위 2부    │                │ │
│ │   │  62,100표    │ ← ⚔️ → │  60,900표    │                │ │
│ │   │  [투표하기]  │         │  [투표하기]  │                │ │
│ │   └──────────────┘         └──────────────┘                │ │
│ │                                                             │ │
│ │   "승격까지 단 1,200표! 지금 역전하세요!"                  │ │
│ │                                                             │ │
│ └─────────────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│ [Tab Content - 선택된 리그 표시]                                │
│                                                                 │
│ ═══════════════════════════════════════════════════════════════ │
│ ▼ [1부 리그 선택 시]                                           │
│ ═══════════════════════════════════════════════════════════════ │
...
│ │  16. Jellyfish Ent.       45,120표   [투표]                │ │
│ │  17. Play M Ent.          42,890표   [투표]                │ │
│ │  18. KQ Entertainment     40,560표   [투표]                │ │
│ │  19. AOMG                 38,230표   [투표]                │ │
│ │  20. P Nation             35,900표   [투표]                │ │
│ │  ...                                                       │ │
│ │                                                             │ │
│ │  [더 보기 ▼] (페이지네이션)                                │ │
│ │                                                             │ │
...
#### Tab Navigation (탭 네비게이션) ⭐ NEW

| 요소              | 설명                                         |
| ----------------- | -------------------------------------------- |
| **1부 리그 탭**   | "🏆 Premier League" - 1~10위 표시            |
| **2부 리그 탭**   | "🔥 Challengers" - 11위~ 표시                |
| **Active 상태**   | 선택된 탭 하이라이트 (언더라인 or 배경색)    |
| **뱃지 표시**     | 각 탭에 소속사 수 표시 (예: "10개", "20개+") |
| **스와이프 지원** | 모바일에서 좌우 스와이프로 탭 전환           |

#### Promotion Battle (승강전 영역) - 항상 표시

| 요소               | 설명                                    |
| ------------------ | --------------------------------------- |
| **고정 위치**      | 탭 아래, 콘텐츠 위에 항상 노출          |
| **양쪽 대결 구도** | 10위(강등위기) vs 11위(승격기회) 시각화 |
| **격차 표시**      | 중앙에 GAP 숫자 강조 (예: "1,200표")    |
| **투표 CTA**       | 양쪽 모두 투표 버튼 제공                |
| **긴장감 연출**    | ⚔️ 아이콘, 펄스 애니메이션              |

#### Premier League (1부 리그 탭)
...
| 요소          | 설명                              |
| ------------- | --------------------------------- |
| **11위 강조** | "승격까지 N표!" 메시지, 카드 형태 |
| **12위 이하** | 리스트 형태, 페이지네이션         |
| **동기 부여** | "승격을 노려라!" 문구             |
| **더 보기**   | 무한 스크롤 또는 페이지 버튼      |
...
│   └──────────────────┘  └──────────────────┘     │
├───────────────────────────────────────────────────┤
│ [Promotion Battle - 고정 영역]                    │
│   10위 RBW  ←  GAP: 1,200표  →  11위 IST         │
├───────────────────────────────────────────────────┤
│                                                   │
│ [선택된 탭 콘텐츠]                                │
│                                                   │
...
// packages/kcl/src/messages/ko.json
{
  "League": {
    ...
    "tabs": {
      "premier": "1부 리그",
      "challengers": "2부 리그",
      "swipe_hint": "스와이프하여 리그 전환"
    },
    "premier": {
      "title": "Premier League",
      "subtitle": "1부 리그",
      "relegation_zone": "강등 위기",
      "relegation_warning": "11위와 {count}표 차이"
    },
    "challengers": {
      "title": "Challengers",
      "subtitle": "2부 리그",
      ...
      "promotion_cta": "지금 밀어주세요!",
      "show_more": "더 보기"
    },
    "promotion_battle": {
      "title": "승강전",
      "gap": "격차",
      "gap_message": "승격까지 단 {count}표!",
      "cta": "지금 역전하세요!"
    },
    "vote": {
      "button": "투표하기",
...
│   ├── LeagueTabs/                     # ⭐ NEW - 탭 네비게이션
│   │   ├── index.tsx                   # 1부/2부 탭 컨테이너
│   │   └── LeagueTabs.module.scss
│   ├── PremierLeague/
│   │   ├── index.tsx                   # 1부 리그 탭 콘텐츠
...
│   ├── PromotionBattle/
│   │   ├── index.tsx                   # 승강전 영역 (항상 표시)
│   │   └── PromotionBattle.module.scss
│   ├── Challengers/
│   │   ├── index.tsx                   # 2부 리그 탭 콘텐츠
...
```typescript
// LeagueTabs ⭐ NEW
type LeagueTabType = 'premier' | 'challengers';

interface LeagueTabsProps {
  activeTab: LeagueTabType;
  onTabChange: (tab: LeagueTabType) => void;
  premierCount: number; // 1부 소속사 수 (항상 10)
  challengersCount: number; // 2부 소속사 수
}
...
// PromotionBattle (항상 표시)
interface PromotionBattleProps {
  battle: PromotionBattle;
  onVote: (companyId: string) => void; // 양쪽 모두 투표 가능
}
...
// useLeagueTab 훅
interface UseLeagueTabReturn {
  activeTab: LeagueTabType;
  setActiveTab: (tab: LeagueTabType) => void;
  swipeHandlers: SwipeHandlers; // 모바일 스와이프 지원
}
```
